...
+ (UIImage *)md_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self md_frameDurationAtIndex:i source:source];
...
+ (float)md_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)md_animatedGIFNamed:(NSString *)name {
    ...
    if (scale > 1.0f) {
        ...
        if (data) {
            return [UIImage md_animatedGIFWithData:data];
        }
